...
const BalancesTabWrapper = styled.div`
    transition: 0.3s all;
    width: ${props => props.opened ? "100%" : "22%"};
    align-self: center;
    height: 52px;
    padding: 5px 10px;
    ...
    border-radius: 25px;
    display: grid;
    grid-template-columns: ${props => props.mobile ? "1fr 2fr" : "1fr 2fr 1fr auto"};
...
export const BalancesTab = () => {
    ...
    return (
        <BalancesTabWrapper opened={balancesExpanded} mobile={isMobileScreen} onClick={() => isMobileScreen ? setBalancesMobileExpanded(true) : setBalancesExpaned(!balancesExpanded)}> 
            <>
                {account ? 
                    <>
            ...
            {isMobileScreen ? 
                <>
                    ...
                    <BalanceExpand opened={balancesMobileExpanded}>
                            <BalanceOverall> 
                                <img src={theme === "light" ? pig_icon_light : pig_icon} width={35} height={35} alt="balance"/>
                                <h5> Balance </h5>
