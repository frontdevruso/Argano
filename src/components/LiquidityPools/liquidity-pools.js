                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart width={100} height={100}>
                            ...
                            {data.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={index === 0 ? "url(#colorUv)" : "url(#colorUvSecond)"} />
                            ))}
                            </Pie>
                        </PieChart>
                    </ResponsiveContainer>
                        ...
                        {liquidityPools.map((item) => {
                            ...
                            return(
                                <li className={`${windowExpanded ? "luqidity-pools-wrapper-list-item liq-item-opened" : "luqidity-pools-wrapper-list-item"}`}> 
                                    ...
                                    <h3> {item.apy} </h3> 
                                    <button className='chart-expand' onClick={() => handleLiquidityPoolsOpened(item.firstToken + item.secondToken)}> {windowExpanded ? <i class="fas fa-times"></i> : <i class="fas fa-chart-line"></i>} </button>
                                    {windowExpanded ? 
                                        <div className='control-panel'> 
                                            <div className='control-panel__header'> 
                                                <button onClick={() => setItemChoosenWindow("Provide Liquidity")} className={`${itemChoosenWindow === "Provide Liquidity" ? "active" : ""}`}> Provide Liquidity </button>
                                                <button onClick={() => setItemChoosenWindow("Volume")} className={`${itemChoosenWindow === "Volume" ? "active" : ""}`}> Volume </button>
                                                <button onClick={() => setItemChoosenWindow("Liquidity")} className={`${itemChoosenWindow === "Liquidity" ? "active" : ""}`} >  Liquidity </button>
                                            </div>
